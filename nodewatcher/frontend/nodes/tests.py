...
from frontend.nodes.models import Pool, PoolStatus
...
def _concurrent_allocation_worker(pool):
  ...
  return (alloc.network, alloc.cidr)
...
class PoolTestCase(unittest.TestCase):
  def setUp(self):
    ...
    self.small_pool = Pool.objects.create(
      family = 4,
      network = "192.168.1.0",
      cidr = 26,
    )
  ...
  def test_basic_allocation(self):
    ...
    self.assertEqual(a.cidr, 27)
    self.assertEqual(b.cidr, 27)
    self.assertEqual(c.cidr, 26)
    ...
    self.assertEqual(a.status, PoolStatus.Full)
    self.assertEqual(b.status, PoolStatus.Full)
    self.assertEqual(c.status, PoolStatus.Full)
  ...
  def test_freeing(self):
    ...
    self.assertEqual(c.cidr, a.cidr)
