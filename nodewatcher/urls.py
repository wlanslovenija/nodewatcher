...
account_patterns = patterns('',
  ...
  url(r'^activate/(?P<activation_key>\w+)/$', decorators.anonymous_required(function=registration_views.activate), {
      'backend': 'nodewatcher.account.regbackend.ProfileBackend',
  ...
  url(r'^login/$', 'nodewatcher.account.views.login', name='auth_login'),
  url(r'^logout/$', 'nodewatcher.account.views.logout_redirect', name='auth_logout'),
  ...
  url(r'^$', 'nodewatcher.account.views.account', name='user_account'),
)

urlpatterns = patterns('',
  # Node lists
  url(r'^$', 'nodewatcher.nodes.views.nodes', name = 'nodes_list'),

  # Node maintainers
  url(r'^my/new$', 'nodewatcher.nodes.views.node_new', name = 'new_node'),

  # Node itself, public
  # (Those views should have permalinks defined and are also those which have be_robust set to True)
  url(r'^node/(?P<node>[^/]+)$', 'nodewatcher.nodes.views.node_display', name = 'view_node'),

  # Node manipulation
  url(r'^node/(?P<node>[^/]+)/edit$', 'nodewatcher.nodes.views.node_edit', name = 'edit_node'),

  # Registry
  (r'^registry/', include('nodewatcher.registry.urls', namespace = 'registry')),

  # Accounts
  (r'account/', include(account_patterns)),
  url(r'^user/(?P<username>[\w.@+-]+)/$', 'nodewatcher.account.views.user', name = 'user_page'),
)
