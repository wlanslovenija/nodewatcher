...
class Node(models.Model):
  ...
  def get_primary_ip_pool(self):
    ...
    """
    try:
      return pool_models.Pool.objects.ip_filter(ip_subnet__contains = "%s/32" % self.ip).get(parent = None)
...
class Subnet(models.Model):
  ...
  def is_from_known_pool(self):
    ...
    return pool_models.Pool.objects.ip_filter(ip_subnet__contains = '{0}/{1}'.format(self.subnet, self.cidr)).count() > 0
