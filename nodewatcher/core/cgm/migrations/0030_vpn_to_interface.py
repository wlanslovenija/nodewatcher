...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        per_node = {}
        for server in orm['cgm.VpnServerConfig'].objects.all():
            per_node.setdefault(server.root.uuid, {}).setdefault(server.hostname, []).append(server)

        vpn_iface_cfg_ctype = self.get_content_type(orm, "cgm", "vpninterfaceconfig")
        vpn_net_cfg_ctype = self.get_content_type(orm, "cgm", "vpnnetworkconfig")
        for node_id, groups in per_node.iteritems():
            used_ifaces = []
            for servers in groups.values():
                try:
                    iface = orm['cgm.VpnInterfaceConfig'].objects.filter(root__pk = node_id).exclude(pk__in = used_ifaces)[0]
                    used_ifaces.append(iface.pk)
                except IndexError:
                    iface = orm['cgm.VpnInterfaceConfig'](content_type = vpn_iface_cfg_ctype)

                iface.routing_protocol = "olsr"

                for server in servers:
                    if not iface.protocol:
                        iface.protocol = server.protocol
                        iface.root = server.root
                        iface.save()

                    netcfg = orm['cgm.VpnNetworkConfig'](content_type = vpn_net_cfg_ctype, root = server.root)
                    netcfg.interface = iface
                    netcfg.address = server.hostname
                    netcfg.port = server.port
                    netcfg.save()
