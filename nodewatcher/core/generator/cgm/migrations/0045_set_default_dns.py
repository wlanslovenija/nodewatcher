...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        dns_ctype = self.get_content_type(orm, app_label='cgm', model='dnsserverconfig')

        # Setup default DNS servers for all nodes; this is for migration from nodewatcher v2
        # where the DNS servers had been hardcoded
        for node in orm['core.Node'].objects.all():
            server = orm['cgm.DnsServerConfig'](root=node, polymorphic_ctype=dns_ctype)
            server.address = '10.254.0.1'
            server.save()

            server = orm['cgm.DnsServerConfig'](root=node, polymorphic_ctype=dns_ctype)
            server.address = '10.254.0.2'
            server.save()
