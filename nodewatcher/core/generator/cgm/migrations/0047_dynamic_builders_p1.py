...
class Migration(SchemaMigration):
    ...
    depends_on = (
        ('generator', '0003_dynamic_builders'),
    )

    def forwards(self, orm):
        # Deleting field 'CgmGeneralConfig.version'
        db.delete_column(u'cgm_cgmgeneralconfig', 'version')

        # Adding field 'CgmGeneralConfig.build_channel'
        db.add_column(u'cgm_cgmgeneralconfig', 'build_channel',
                      self.gf('nodewatcher.core.registry.fields.ModelSelectorKeyField')(to=orm['generator.BuildChannel'], null=True, on_delete=models.PROTECT, blank=True),
                      keep_default=False)
    ...
    def backwards(self, orm):
        ...
        db.add_column(u'cgm_cgmgeneralconfig', 'version',
                      self.gf('nodewatcher.core.registry.fields.SelectorKeyField')(default='', max_length=50, regpoint='node.config', enum_id='core.general#version', blank=True),
                      keep_default=False)

        # Deleting field 'CgmGeneralConfig.build_channel'
        db.delete_column(u'cgm_cgmgeneralconfig', 'build_channel_id')
