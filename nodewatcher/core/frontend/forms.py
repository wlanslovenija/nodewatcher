...
from . import (add_css_class, render_tag, split_css_classes,
               render_icon, text_concat, text_value, get_theme_setting,
               get_form_renderer, get_field_renderer, get_formset_renderer)
...
def render_field_and_label(
    ...
    if layout == 'horizontal':
        if not label_class:
            label_class = get_theme_setting('horizontal_label_class')
        if not field_class:
            field_class = get_theme_setting('horizontal_field_class')
        if not label:
            label = '&#160;'
...
def is_widget_required_attribute(widget):
    ...
    """
    if not get_theme_setting('set_required'):
        return False
