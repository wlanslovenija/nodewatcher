...
def configure_leasable_network(cfg, node, interface, network, iface_name, subnet):
    ...
    if network.nat_type == 'snat-primary-ip':
        ...
        try:
            ...
            raise cgm_base.ValidationError(
                _("SNAT using primary IP as source address configured, but router ID is missing! The node must have a configured primary IP address.")
            )
