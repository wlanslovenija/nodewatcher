...
class NodeConfig(object):
  ...
  subnets = None
  vpn = None
  vpnServer = (("46.54.226.43", 9999), ("46.54.226.49", 9999))
  tdServer = [("46.54.226.43", [8942, 53, 123]), ("92.53.140.74", [8942, 53, 123])]
...
""".format(broker_ports = ports, uuid = self.uuid, iface = iface))
      ...
      # QoS configuration
      f = open(os.path.join(configPath, "qos"), 'w')
      if self.vpn and self.vpn['limit_up']:
        for iface, _ in enumerate(self.tdServer):
          f.write("config interface digger%d\n" % iface)
          f.write("        option classgroup  'Default'\n")
...
""".format(
  ...
  def build(self, path):
    ...
    if self.openwrtVersion == 'hardcoded-tpl-trunk':
      ...
      buildString = 'make image FILES="../files" PROFILE="%s" PACKAGES="policy-routing olsrd uhttpd tc nodewatcher-core nodewatcher-clients ntpclient -ppp -ppp-mod-pppoe wpad-mini kmod-l2tp kmod-l2tp-ip kmod-l2tp-eth tunneldigger wireless-tools qos-scripts nodewatcher-watchdog %s"' % (profile_map[self.portLayout], pkgs)
      os.chdir(path)
