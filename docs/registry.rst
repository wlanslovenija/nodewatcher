...
The resulting `Node` instances will have two additional fields called `name` and `router_id` that will have the values of their respective registry models. The `name` field will be obtained by joining with the `GeneralConfig` table and `router_id` by joining with the `RouterIdConfig` model. Joining is done by the registry via standard Django ORM APIs.